<template>
	<div class="comment-manage view">
		...
		<div v-if="loadOK">
			<BackCommentList @reply="handleSendReply" @del="handleDel" v-for="item in commentList" :key="item.comment_id" :info="item"></BackCommentList>
		</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			handleDel(id, info) {
				delComment({'params': {id, aid: info.article_id}}).then(() => {
					this.reload();
				}).catch(err => {
					console.log(err);
				});
