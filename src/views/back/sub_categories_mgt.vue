	...
	<div class="sub-categories view">
		...
		<div class="sub-line">
			...
			<select v-model="selectedCategories" v-if="isShowed">
				...
				<option v-for="item in categoriesList" :value="item.categories_id" :key="item.id">{{ item.categories_name }}</option>
			</select>
	...
	methods: {
		...
		exec() {
			...
			if (parseInt(this.selectedOption) === 2 && this.checkedArticle.length) {
				...
				delArticles({'params': params}).then(() => {
					this.reload();
				}).catch(err => {
					console.log(err);
				});
